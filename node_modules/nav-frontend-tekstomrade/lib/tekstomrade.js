...
var notNull = function notNull(element) {
    return element !== null;
};
function tilJSXLenke(fragment, fragmentIndex) {
    var urimatch = uriRegex.exec(fragment);
    if (urimatch === null) {
...
function leggTilLenkerJSX(innhold) {
    var fragments = innhold.split(uriRegex);
    ...
    return fragments.map(tilJSXLenke).filter(notNull);
}
...
var Tekstomrade = function (_React$Component) {
    _inherits(Tekstomrade, _React$Component);

    function Tekstomrade() {
        _classCallCheck(this, Tekstomrade);

        return _possibleConstructorReturn(this, (Tekstomrade.__proto__ || Object.getPrototypeOf(Tekstomrade)).apply(this, arguments));
    }

    _createClass(Tekstomrade, [{
        key: "render",
        value: function render() {
            var _props = this.props,
                children = _props.children,
                ingenFormattering = _props.ingenFormattering,
                props = _objectWithoutProperties(_props, ["children", "ingenFormattering"]);

            var tekst = children;
            var avsnitt = ingenFormattering ? tekst : tekst.split(/[\r\n]+/).map(leggTilLenkerJSX).filter(notNull).map(tilAvsnitt);
